...
def main():
    ...
    first = pd.to_datetime(args.from_date)
    last = pd.to_datetime(args.to_date)
    ...
    # labeled_features.iloc[0:50000].to_csv(args.out, index=False)
    # labeled_features.to_csv(args.out, index=False)

    print("cutting...")
    labeled_features.loc[(first <= labeled_features['datetime']) & (labeled_features['datetime'] <= last)].to_csv(args.out, index=False)
